...
//     uint8_t addressTagField = mainMemoryAddress & MAIN_MEMORY_BLOCK_ADDDRESS_BITMASK;
...
//     bool isCacheSetValid = cachePtr->cacheSets[cacheSet].validBit;
//     bool tagFieldsMatch = addressTagField == cachePtr->cacheSets[cacheSet].tag;
//     while (tagFieldsMatch!=true){
//         cacheSet++;
//         if (cacheSet>=4) {
//             cacheSet =0;
//             break;
//         }
//         tagFieldsMatch = addressTagField == cachePtr->cacheSets[cacheSet].tag;

//     }
//     if (cacheSet==0 && tagFieldsMatch !=true ){
//         while(isCacheSetValid!=false){
//             cacheSet++;
//             if(cacheSet>3){
//                 cacheSet =0;
//                 break;
//             }
//             isCacheSetValid = cachePtr->cacheSets[cacheSet].validBit;
//         }
//     }
...
//         cachePtr->cacheSets[cacheSet].blockValues[0] = cachePtr->mainMemoryPtr->values[memoryBlockAddress];
//         cachePtr->cacheSets[cacheSet].blockValues[1] = cachePtr->mainMemoryPtr->values[memoryBlockAddress + 1];
//         cachePtr->cacheSets[cacheSet].blockValues[2] = cachePtr->mainMemoryPtr->values[memoryBlockAddress + 2];
//         cachePtr->cacheSets[cacheSet].blockValues[3] = cachePtr->mainMemoryPtr->values[memoryBlockAddress + 3];

//         //Update the cache set tag field and valid bit
//         cachePtr->cacheSets[cacheSet].tag = addressTagField;
//         cachePtr->cacheSets[cacheSet].validBit = true;
//     }
//     //Read the valid bit and the tag from the cache set
